...
export function getInfrastructureBounces(
  limit: number = 20,
  offset: number = 0,
  isRead?: boolean,
) {
...
export function getTlsMailCerts() {
  return sudoApi.get<{
    current: TlsCertificate | null;
    latest: TlsCertificate | null;
  }>({
  ...
  return sudoApi
    ...
    .then(({ sends, projects }) => {
      ...
      return sends.map(({ project_id, ...send }) => ({
        ...send,
        project: projectsById.get(project_id) ?? {
          id: project_id,
...
export function getWarmupSchedules(ipAddressId?: number) {
  return sudoApi.get<WarmupSchedule[]>({
    ...
    data:
      ipAddressId !== undefined ? { ip_address_id: ipAddressId } : undefined,
  });
}
